<template>
  <div class="city-wrap">
    <Header go-back :head-title="currentCity">
      <router-link slot="changecity" class="changeCity" to="/index">切换城市</router-link>
    </Header>
    <section class="form-box">

      <cube-input v-model="inputVaule" clearable placeholder="输入学校、商务楼、地址"></cube-input>
...
export default {
  ...
  methods: {
    ...
    nextPage(index, hash) {
      const history = getStore('HISTORYDATA')
      const item = this.placelist[index]
      if (history) {
